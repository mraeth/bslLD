...
function gpu_cartesian_grid()
    return bslLD.Grid([0.0, -1.5], [2pi, 1.5], [8, 6], 1)
end

gpu_simulation_time() = bslLD.SimulationTime(0.05, 0.05)
...
@testset "GPU Regression" begin
    ...
    if cuda_loaded && bslLD.cuda_available()
        ...
        @test_nowarn bslLD.advectV!(f_gpu_v, grid, simTime, e_gpu)
