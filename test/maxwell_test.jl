...
@testset "Maxwell CN Vacuum" begin
    ...
    E = bslLD.VectorField([zeros(length(x)), sin.(x), zeros(length(x))])
    B = bslLD.VectorField([zeros(length(x)), zeros(length(x)), sin.(x)])
    ...
    diagonal = 1.0 .- alpha2
    curlEhat =
        [zero.(Ehat_before[1]), -im .* kx .* Ehat_before[3], im .* kx .* Ehat_before[2]]
    curlBhat =
        [zero.(Bhat_before[1]), -im .* kx .* Bhat_before[3], im .* kx .* Bhat_before[2]]
    Ehat_expected = [
        prefac .* (diagonal .* Ehat_before[d] .+ params.c^2 * dt .* curlBhat[d]) for d = 1:3
    ]
    Bhat_expected =
        [prefac .* (diagonal .* Bhat_before[d] .- dt .* curlEhat[d]) for d = 1:3]
