    ...
    for iv1 = 2:size(f.data)[2]
        for iv2 = 1:size(f.data)[3]
            xdisp =
                vth *
                grid.vaxes[1][iv1] *
                cos(grid.vaxes[2][iv2] + grid.b0 * grid.time[grid.index[1]])
...
function advectV!(
    ...
    @threads for ix = 1:nx
        ...
        delta_mux =
            delta_vx * cos(grid.b0 * grid.time[grid.index[1]]) +
            delta_vy * sin(grid.b0 * grid.time[grid.index[1]])
        delta_muy =
            -delta_vx * sin(grid.b0 * grid.time[grid.index[1]]) +
            delta_vy * cos(grid.b0 * grid.time[grid.index[1]])

        # 1. Build Splines
        itp_re = Vector{Dierckx.Spline1D}(undef, imax + 1)
        itp_im = Vector{Dierckx.Spline1D}(undef, imax + 1)
        @inbounds for m = 1:(imax+1)
            vals_re = real.(F[ix, :, m])
            vals_im = imag.(F[ix, :, m])
        ...
        @inbounds for iv = 1:nv
            vr_grid = vgrid[iv]
            @inbounds for ip = 1:nphi
                phi_grid = phigrid[ip]
                displace_polar_velocity!(
                    vrp_ref,
                    phip_ref,
                    vr_grid,
                    phi_grid,
